    ...
    public class MqttNetworkChannel : IMqttNetworkChannel
    {
        ...
        public void Connect()
        {
            ...
            if (secure)
            {
                ...
                // Modified to always trust server certificate (for self-signed CA)
                // and to explicitly select our client certificate
                X509Certificate localCert = this.clientCert;
                this.sslStream = new SslStream(
                    this.netStream,
                    false,
                    (sender, certificate, chain, errors) => true,
                    (sender, targetHost, localCerts, remoteCert, acceptableIssuers) => localCert);
#endif
        ...
        public static SslProtocols ToSslPlatformEnum(MqttSslProtocols mqttSslProtocol)
        {
            switch (mqttSslProtocol)
            {
                ...
                case MqttSslProtocols.TLSv1_1:
                    return (SslProtocols)768;  // SslProtocols.Tls11
                case MqttSslProtocols.TLSv1_2:
                    return (SslProtocols)3072; // SslProtocols.Tls12
                default:
                    return SslProtocols.None; // let OS negotiate
            }
        }
